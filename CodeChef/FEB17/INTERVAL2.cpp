...
int main()
{
	...
	while(T--)
	{
		...
		int now = M&1, last = !now;
		for (int i=1;i+B[M]-1<=N;i++)f[now][i] = sum(i,B[M]);
		for (int i=M-1;i>=1;i--)
		{
			now^=1;
			last^=1;
			head = 1;
			tail = 0;
			for (int j=N-B[i+1];j>N-B[i]+1;j--)//123456
			{
			...
			for (int j=N-B[i]+1;j>=1;j--)
			{
				...
				f[now][j] = sum(j,B[i]) - f[last][q[head]];
				//printf("%d %d %d\n",j,q[head],f[last][q[head]]);
				while(tail >= head && f[last][j] >= f[last][q[tail]]) tail--;
				q[++tail] = j;
			}
