...
namespace MODULE{
	...
	struct Point {
	  ...
	  ft operator*(const Point &a) const {
	    return x * a.x + y * a.y;
	  }
	...
	bool samedir(Point a, Point b, Point x, Point y) {
	...
	int CircumCircle(ft xp, ft yp, ft x1, ft y1, ft x2, 
	    ft y2, ft x3, ft y3, ft &xc, ft &yc, ft &r){
	  ft m1, m2, mx1, mx2, my1, my2;
	  ft dx, dy, rsqr, drsqr;
	...
	int Triangulate(int nv, Point pxyz[], ITRIANGLE v[], int &ntri){
	  int *complete = NULL;
	  EDGE *edges = NULL; 
	  EDGE *p_EdgeTemp;
	  int nedge = 0;
	  ...
	  int inside;
	  int i, j, k;
	  ft xp, yp, x1, y1, x2, y2, x3, y3, xc, yc, r;
	  ft xmin, xmax, ymin, ymax, xmid, ymid;
	  ft dx, dy, dmax; 
...
int n;
Point p[MaxVertices + 10];
ITRIANGLE v[40 * MaxVertices];
Point pfrom, pto;
Point cir_o[40 * MaxVertices];
...
namespace G
{
	vector<pair<int, ft> > e[N];
	ft dis[N];
	int vis[N];
	int n;
	void init(int _n) {
		n = _n;
		for (int i = 0; i < N; ++i) e[i].clear(),dis[i] = inf, vis[i] = false;
		fill_n(dis, N, 0);
		fill_n(vis, N, 0);
	}
	
	void add(int x, int y, ft v) {
...
int main() {
  ...
  //freopen("14.submit.out","w",stdout);
    ...
    qsort(p, n, sizeof(Point), PointCompare);
    int ntri;
